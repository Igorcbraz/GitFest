...
const calc = (x: number, y: number) => [-(y - window.innerHeight / 2) / 40, (x - window.innerWidth) / 35, 1];
          ...
          <div className='flex flex-col justify-between items-center'>
            <div className='flex flex-col items-start mt-10 md:mt-16'>
              <h1 className='text-[1.7rem] md:text-[3rem] font-bold text-gray-900 dark:text-gray-300'>
                Create a <span className='text-primary-400'>festival lineup</span> <br /> from your top repositories.
              </h1>
              ...
              <div className='w-full flex flex-col justify-center items-center gap-5'>
                <form
                  className='flex justify-between items-center mt-10 gap-3 w-full'
                  onSubmit={(e) => {
                    e.preventDefault();
                ...
                <span className='w-full flex justify-center items-center'>
                  <button
                    className={`group/btngithub flex justify-evenly items-center w-1/2 h-12 shadow-lg shadow-primary-100 transition text-white font-bold py-3 px-4 rounded-lg gap-4 md:px-6 md:w-2/5 dark:shadow-zinc-800 ${hasSupabase ? 'bg-primary-500 hover:bg-primary-400' : 'bg-gray-400 cursor-not-allowed'}`}
                    onClick={() => hasSupabase && signIn(true)}
                  >
                    Login With Github
                    <Github iconClassname='w-5 h-5 ml-0 fill-current md:ml-2' />
                  </button>
                </span>
              </div>
            </div>
